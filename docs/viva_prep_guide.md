# 🎓 PVG ERP Authentication Module — Viva Exam Preparation Guide
...
* **Answer:** 
  * Changed the first input label from `REGISTERED EMAIL-ID` $ightarrow$ `Email`.
  * Changed the second input label from `Secret Key` (Admin) / `Security Credentials` (User) $ightarrow$ `Password`.
